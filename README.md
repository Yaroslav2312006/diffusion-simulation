...
Реализован в рамках учебной ознакомительной практики (вариант А-09) по направлению 09.03.04 «Программная инженерия».
...
Запустите моделирование с параметрами по умолчанию (1000 частиц, 100 шагов, шаг 1.0):

bash
python -m src.main
Чтобы изменить параметры, используйте аргументы командной строки (см. раздел Параметры).
...
bash
docker run -it --rm -v ${PWD}/output:/app/output diffusion-sim
После завершения работы откройте папку output — там будут PNG-файлы и CSV с результатами.
