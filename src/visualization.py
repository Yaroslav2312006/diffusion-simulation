...
def plot_histogram(distances: np.ndarray,
                   bins: int = 20,
                   sigma: Optional[float] = None,
                   title: str = "Распределение расстояний",
                   save_path: Optional[str] = None) -> None:
    """
    ...
    plt.bar(bin_centers, hist, width=bin_edges[1] - bin_edges[0],
            alpha=0.6, color='skyblue', edgecolor='navy', label='Эксперимент')
...
def plot_msd(experimental_msd: np.ndarray,
             step_size: float = 1.0,
             title: str = "Среднее квадратичное смещение",
             save_path: Optional[str] = None) -> None:
    """
