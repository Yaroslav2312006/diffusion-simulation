...
def main():
    ...
    parser.add_argument('-n', '--particles', type=int, default=1000,
                        help='Количество частиц (по умолчанию 1000)')
    parser.add_argument('-s', '--steps', type=int, default=100,
                        help='Число шагов моделирования (по умолчанию 100)')
    parser.add_argument('-l', '--step-size', type=float, default=1.0,
                        help='Длина шага (по умолчанию 1.0)')
    parser.add_argument('-o', '--output-dir', type=str, default='./output',
                        help='Директория для сохранения результатов (по умолчанию ./output)')
    parser.add_argument('--no-viz', action='store_true',
                        help='Отключить визуализацию (только экспорт)')
    ...
    if not args.no_viz:
        ...
        plot_trajectories(trajectories, num_particles=10,
                          title=f"Траектории частиц (N={args.particles})",
                          save_path=traj_path)
        print(f"Траектории сохранены в {traj_path}")

        # Гистограмма распределения
        hist_path = os.path.join(args.output_dir, 'histogram.png')
        plot_histogram(distances, bins=20, title="Распределение расстояний",
                       save_path=hist_path)
        print(f"Гистограмма сохранена в {hist_path}")

        # График MSD
        msd_path = os.path.join(args.output_dir, 'msd.png')
        plot_msd(msd, args.step_size, title="Среднее квадратичное смещение",
                 save_path=msd_path)
